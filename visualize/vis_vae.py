...
def plt_scatter(df, name, exp, out_dir):
    ...
    fig.savefig(os.path.join(res_dir, f"{exp}_samples.svg"), bbox_inches='tight')
    plt.clf()
    ...
    res_dir = os.path.join(out_dir, name, "samples")
    os.makedirs(res_dir, exist_ok=True)
    fig.savefig(os.path.join(res_dir, f"{exp}_samples_ae.svg"), bbox_inches='tight')
    plt.clf()
...
if __name__ == '__main__':
    ...
    names = [
        ...
        ("vsae", "0005", models.SupCVAE),
        ("vsas", "0005", models.SupCVAENS),
        ("vsaes", "0004", models.SupCVAENS),
    ]
